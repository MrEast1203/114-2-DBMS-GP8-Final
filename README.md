...
A research-grade benchmark of four query-planning strategies for running
...
implements increasingly sophisticated plans (`naive` → `v0` → `v1` →
`v2`) that culminate in **graph filter push-down into the ranker SQL**.
...
| plan  | mean P50 | NDCG@10 | top-10 vs v0          |
| ----- | -------- | ------- | --------------------- |
| naive | 51.36 ms | 0.675   | Jaccard 1.000 (same)  |
| v0    | 51.19 ms | 0.675   | Jaccard 1.000 (same)  |
...
- `naive`, `v0`, and `v1` return **bit-identical top-10s** on every
  query, demonstrating that RRF is commutative over engine execution
  order when the engine set is fixed.
...
│   └── src/
│       ├── plan.rs         # naive / v0 / v1 / v2 plan implementations
...
├── scripts/            # Helper scripts (e.g. coldwarm_all_28.py)
...
| capability                                | naive | v0 | v1 | v2 |
| ----------------------------------------- | ----- | -- | -- | -- |
| RRF fusion across engines                 |  ✓   | ✓ | ✓ | ✓ |
| reorder engines by selectivity            |       | ✓ | ✓ | ✓ |
| BFS cost = `branching^depth` (geometric)  |       |    | ✓ | ✓ |
...
| file                              | purpose                                        |
| --------------------------------- | ---------------------------------------------- |
| `eval_phase1_e4.json`             | 20 queries × 4 plans · P50 + NDCG + Jaccard + RBO |
| `coldwarm_full_28.json`           | 7 queries × 4 plans · cold-vs-warm P50 matrix  |
| `storage.json`                    | per-relation / per-index disk usage breakdown  |
| `bfs_shootout.json`               | AGE Cypher vs `WITH RECURSIVE`, 3 depth × 3 bucket |
| `coldwarm_q{1..7}_{naive,v0,v1,v2}.json` | individual cells of the cold/warm matrix |
...
uv run python scripts/coldwarm_all_28.py
